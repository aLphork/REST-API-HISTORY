...
router.get('/getAll', authenticateToken, async (req, res) => {
  try{
    ...
    if( !users || users.length === 0 ) return res.status(500).json({ message: 'Kullanicilar bulunamadı.'});
...
router.put('/update/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (user.changeCount >= 3) {
      await user.destroy();
      return res.status(403).json({ message: '3 değişiklik oldu kullanici silindi.' });
    }
